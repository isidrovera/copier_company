...
class CopierCompany(http.Controller):
    ...
    def copier_company_form(self, **kwargs):
        try:
            ...
            _logger.info(
                'Form loaded with %s marcas and %s tipos_identificacion',
                len(marcas), len(tipos_identificacion)
            )
    ...
    # 🔴 AQUÍ ESTABA EL WARNING → CORREGIDO
    @http.route('/copier_company/get_customer_data', type='jsonrpc', auth="public")
    def get_customer_data(self, **kwargs):
        ...
        _logger.debug(
            'Searching for customer with ID type %s and ID %s',
        ...
        try:
            ...
            new_partner = request.env['res.partner'].sudo().create({
                ...
                ].search([('l10n_pe_vat_code', '=', tipo_identificacion)], limit=1).id,
                'name': 'Cargando...',
            })

            # Actualizar datos desde onchange
