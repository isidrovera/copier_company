...
class CopierQuotation(models.Model):
    ...
    fecha_cotizacion = fields.Date('Fecha de Cotización', default=fields.Date.context_today, required=True, tracking=True)
    ...
    tipo_identificacion = fields.Many2one(
        related='cliente_id.l10n_latam_identification_type_id',
    ...
    linea_equipos_ids = fields.One2many(
        'copier.quotation.line', 'quotation_id',
        string='Equipos Cotizados', copy=True)

    # Modalidad de pago y configuración
    modalidad_pago_id = fields.Many2one(
        'copier.payment.mode', string='Modalidad de Pago',
        required=True, tracking=True)

    # Fechas del contrato
    fecha_inicio_propuesta = fields.Date('Fecha Inicio Propuesta')
    duracion_contrato_id = fields.Many2one(
        'copier.duracion', string="Duración del Contrato",
        default=lambda self: self.env.ref('copier_company.duracion_1_año', raise_if_not_found=False).id
        if self.env.ref('copier_company.duracion_1_año', raise_if_not_found=False) else False)
    ...
    subtotal_mensual = fields.Monetary(
        'Subtotal Mensual', compute='_compute_totales',
        store=True, currency_field='currency_id')
    descuento_mensual = fields.Monetary(
        'Descuento Mensual', compute='_compute_totales',
        store=True, currency_field='currency_id')
    subtotal_con_descuento_mensual = fields.Monetary(
        'Subtotal Mensual c/Desc', compute='_compute_totales',
        store=True, currency_field='currency_id')
    igv_mensual = fields.Monetary(
        'IGV Mensual', compute='_compute_totales',
        store=True, currency_field='currency_id')
    total_mensual = fields.Monetary(
        'Total Mensual', compute='_compute_totales',
        store=True, currency_field='currency_id')

    # Campos calculados - Totales por modalidad de pago
    subtotal_modalidad = fields.Monetary(
        'Subtotal por Modalidad', compute='_compute_totales',
        store=True, currency_field='currency_id')
    descuento_modalidad = fields.Monetary(
        'Descuento por Modalidad', compute='_compute_totales',
        store=True, currency_field='currency_id')
    descuento_total = fields.Monetary(
        'Descuento Total', compute='_compute_totales',
        store=True, currency_field='currency_id')
    subtotal_final = fields.Monetary(
        'Subtotal Final', compute='_compute_totales',
        store=True, currency_field='currency_id')
    igv_final = fields.Monetary(
        'IGV Final', compute='_compute_totales',
        store=True, currency_field='currency_id')
    total_por_modalidad = fields.Monetary(
        'Total por Modalidad', compute='_compute_totales',
        store=True, currency_field='currency_id')

    # Total anual para referencia
    total_anual = fields.Monetary(
        'Total Anual', compute='_compute_totales',
        store=True, currency_field='currency_id')
...
class CopierQuotationLine(models.Model):
    ...
    quotation_id = fields.Many2one(
        'copier.quotation', string='Cotización',
    ...
    subtotal_bn = fields.Monetary(
        'Subtotal B/N', compute='_compute_subtotales',
        store=True, currency_field='currency_id')
    subtotal_color = fields.Monetary(
        'Subtotal Color', compute='_compute_subtotales',
        store=True, currency_field='currency_id')
    subtotal_linea = fields.Monetary(
        'Subtotal Línea', compute='_compute_subtotales',
        store=True, currency_field='currency_id')

    currency_id = fields.Many2one(
        related='quotation_id.currency_id', string='Moneda', readonly=True)

    # ==========================================================
    # CAMPOS DE PRESENTACIÓN PARA PDF / COTIZACIÓN
    # Representan: precio por cada equipo + IGV.
    # No modifican la lógica actual de subtotal_linea ni totales.
    # ==========================================================
    ...
    def _compute_importes_con_igv(self):
        ...
        for line in self:
            ...
            parcial_sin_igv = precio_unitario_sin_igv * cantidad * tiempo_meses
            igv_parcial = parcial_sin_igv * (igv_porcentaje / 100.0)
            parcial_con_igv = parcial_sin_igv + igv_parcial
            ...
            line.parcial_sin_igv_pdf = parcial_sin_igv
            line.igv_parcial_pdf = igv_parcial
            line.parcial_con_igv_pdf = parcial_con_igv
    ...
    def _onchange_equipo_id(self):
        ...
        if self.equipo_id:
            # Lógica futura para precios sugeridos basados en el modelo
            pass
...
class CopierPaymentMode(models.Model):
    ...
    def _create_default_payment_modes(self):
        ...
        for mode_data in modes:
            if not self.search([('name', '=', mode_data['name'])], limit=1):
...
class CopierCompany(models.Model):
    ...
    cotizacion_origen_id = fields.Many2one(
        'copier.quotation', string='Cotización Origen',
