...
class PCloudConfig(models.Model):
    ...
    def download_pcloud_file(self, file_id):
        """
        Fallback: getfilelink clásico (IP-bound).
        ATENCIÓN: La URL generada está atada a la IP del servidor Odoo (VPS).
        Si el usuario la abre desde su browser con diferente IP, pCloud la rechazará.
        Usar _get_or_create_file_publink() para URLs sin binding de IP.
        """
        self.ensure_one()
        if not self.access_token:
            raise UserError("No hay token. Conéctate a pCloud primero.")
    ...
    def _get_or_create_file_publink(self, file_id):
        """
        ★ NUEVO — Solución al problema de IP binding ★

        Obtiene o crea un link público permanente para un archivo.
        Los pub links NO están atados a IP — funcionan desde cualquier browser.

        Estrategia en cascada:
          1. getfilepublink  → obtener link existente (sin IP binding) ✅
          2. createfilepublink → crear link público nuevo              ✅
          3. download_pcloud_file fallback → IP-bound (⚠️ puede fallar)
        """
        self.ensure_one()
        base = self._get_api_url()

        # 1. Intentar obtener link público existente
        resp = requests.get(f"{base}/getfilepublink", params={
            'access_token': self.access_token,
            'fileid': file_id,
        }, timeout=30)
        data = resp.json()
        _logger.info('[pCloud] getfilepublink fileid=%s result=%s', file_id, data.get('result'))

        if data.get('result') == 0:
            code = data.get('code', '')
            _logger.info('[pCloud] Found existing publink code=%s', code)
            return f"https://u.pcloud.link/publink/show?code={code}"

        # 2. Crear link público nuevo
        _logger.info('[pCloud] No existing publink, creating for file %s', file_id)
        resp2 = requests.get(f"{base}/createfilepublink", params={
            'access_token': self.access_token,
            'fileid': file_id,
        }, timeout=30)
        data2 = resp2.json()
        _logger.info('[pCloud] createfilepublink result=%s data=%s',
                     data2.get('result'), data2)

        if data2.get('result') == 0:
            code = data2.get('code', '')
            _logger.info('[pCloud] Created publink code=%s', code)
            return f"https://u.pcloud.link/publink/show?code={code}"

        # 3. Fallback: getfilelink IP-bound
        _logger.warning(
            '[pCloud] publink creation failed result=%s error=%s — fallback to getfilelink',
            data2.get('result'), data2.get('error')
        )
        return self.download_pcloud_file(file_id)
    ...
    @api.model
    def pcloud_get_credentials(self):
        """
        Devuelve las credenciales mínimas para que el BROWSER pueda llamar
        directamente a la API de pCloud con su propia IP.

        Esto resuelve el problema de IP binding de getfilelink:
        - El servidor NO genera la URL (quedaría atada a la IP del VPS)
        - El browser recibe el token y hace el request con su propia IP
        - pCloud genera la URL para la IP del browser → funciona correctamente

        Seguridad: el access_token de pCloud es de solo lectura para el usuario
        y ya se usa en el frontend implícitamente en los pub links.
        """
        config = self.search([('access_token', '!=', False)], limit=1)
        if not config:
        ...
        return {
            ...
            'hostname': config.hostname or 'api.pcloud.com',
        }
    ...
    def pcloud_get_file_download_url(self, file_id):
        """
        ★ CORREGIDO — Sin binding de IP ★

        Obtiene URL de descarga de un archivo usando pub links (sin IP binding).
        Las URLs generadas funcionan desde cualquier browser, sin importar la IP.

        Estrategia:
          1. getfilepublink  → link público existente (sin IP binding) ✅
          2. createfilepublink → crear link público nuevo              ✅
          3. getfilelink fallback → IP-bound (solo como último recurso) ⚠️
        """
        config = self.search([('access_token', '!=', False)], limit=1)
        if not config:
            raise UserError('No hay configuración de pCloud activa.')
        return config._get_or_create_file_publink(int(file_id))
        ...
        for folder in folders:
            ...
            if not folder_id or not folder_name:
                results.append({
                    ...
                    'status': 'error', 'message': 'Carpeta inválida',
            ...
            document.sudo().write({'attached_on_sale': 'sale_order'})
