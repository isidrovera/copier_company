...
class CopierServiceRequest(models.Model):
    ...
    def action_asignar_tecnico(self):
        ...
        if not self.tecnico_id:
            raise UserError('Debe seleccionar un técnico antes de asignar.')
        ...
        self.activity_schedule(
            ...
            summary=f'Servicio Técnico: {self.tipo_problema_id.name}',
            note=f'''
        ...
        return {
            ...
            'params': {
                'message': f'Servicio asignado a {self.tecnico_id.name}',
                'type': 'success',
                'sticky': False,
            }
        }
    ...
    def action_confirmar_visita(self):
        ...
        if not self.fecha_programada:
            raise UserError('Debe programar una fecha antes de confirmar.')
    ...
    def action_completar_servicio(self):
        ...
        if not self.trabajo_realizado:
            raise UserError('Debe describir el trabajo realizado antes de completar.')
        ...
        # Registrar contador si fue capturado
        if self.contador_bn or self.contador_color:
            self._registrar_contador()
        
        self.message_post(
            body=f'✅ Servicio completado a las {fields.Datetime.now().strftime("%H:%M")}',
            message_type='notification'
        )
